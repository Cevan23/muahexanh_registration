...
public class ProjectService implements iProjectService {
    ...
    @Override
    public Map<String, Object> getProjectByLeaderIdAndProjectIdStudentPending(long leaderId, long projectId) throws Exception {
        if (leaderId == 0 || projectId == 0) {
            // Handle the case where leaderId or projectId is 0
            throw new DataNotFoundException(
                    "Cannot find project with leaderID haha: " + leaderId + " projectId: " + projectId);
        ...
        if (projectOptional.isPresent()) {
            ...
            if (studentsOptional.isPresent()){
                ...
                List<Map<String, Object>> studentsHash=students.stream()
                        .map(student -> {
                            Optional<StudentsResigtrationEntity> studentResEntity = studentResigtrationRepository.findByProjectsIdAndStudentId(projectId,student.getId());
                            Map<String, Object> studentsMap = new HashMap<>();
